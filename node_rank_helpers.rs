...
pub(crate) fn compute_node_rank(
    ...
    let mut node_rank: RankScore = RankScore::default();
    match qsets_containting_node {
        Some(involving_sets) => {
            for set in involving_sets {
                ...
                let pr_sum: RankScore = creators.iter().map(|&v| pr_scores[v] as f64).sum();
...
pub(crate) fn map_quorum_sets_to_generators(fbas: &Fbas) -> HashMap<String, HashSet<NodeId>> {
    ...
    let nodes: Vec<NodeId> = (0..fbas.nodes.len()).collect();
    for v in nodes.iter() {
        let quorum_set_hash = hex::encode(Sha3_256::digest(
            fbas.nodes[*v]
                .quorum_set
                .clone()
                .into_id_string()
...
pub(crate) fn all_quorum_sets_containing_node(node_id: NodeId, fbas: &Fbas) -> HashSet<QuorumSet> {
    ...
    for v in fbas.nodes.iter() {
        if v.quorum_set.contained_nodes().contains(node_id) {
            qsets_containting_node.insert(v.quorum_set.clone());
        }
    }
...
mod tests {
    ...
    fn list_of_generators_for_quorum_set() {
        ...
        let actual =
            get_list_of_creators_for_quorum_set(&fbas.nodes[0].quorum_set, &sets_generators_map);
